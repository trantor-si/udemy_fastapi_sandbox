...
async def update_todo(user_id: int, user: User,
                      db: Session = Depends(get_db)):
    if user is None:
...
async def update_password(user_id: int, new_password: str, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    if user is None:
        raise ModuleException(
            function_name=inspect.currentframe().f_code.co_name,
            message="User not found", status_code=status.HTTP_404_NOT_FOUND)
